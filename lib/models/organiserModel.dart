...
Future<void> imageOrganiserModel(List<FileSystemEntity> entities,List<String> organisation, bool rename, bool duplicates) async {
  Map validName = {};
  //Map <String,List<FileSystemEntity>>duplicatesName = {};
  ...
  for (var element in entities) {
    if (FileManager.isFile(element)) {
      ...
      if (data.isNotEmpty) {
        newFolder =
            await newFolderName(organisation, data);
        bool exists =
            await Directory("${entities[0].parent.path}${newFolder}").exists();
        ...
        if (!exists) {
          ...
          if(duplicates){
            if(newFolder!="" && !duplicatesName.contains(element.parent.path+newFolder)){
...
Future<String> newFolderName(
  ...
  for (var element in organisation) {
    if (element == "Year") {
      name += data['EXIF DateTimeOriginal']
              .toString()
              .replaceAll(" ", ":")
              .split(":")[0] +
          "/";
